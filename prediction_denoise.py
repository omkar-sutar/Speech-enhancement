...
audio_output_prediction, sample_rate, min_duration, frame_length, hop_length_frame, n_fft, hop_length_fft,messageQueue:Queue):
    ...
    messageQueue.put("Loaded model from disk")
    ...
    messageQueue.put("Dimensions of squared spectrogram: "+str(dim_square_spec))
    ...
    messageQueue.put("Predicting noise")
    X_pred = loaded_model.predict(X_in)
    #Rescale back the noise model
    inv_sca_X_pred = inv_scaled_ou(X_pred)
    #Remove noise model from noisy speech
    messageQueue.put("Removing noise")
    X_denoise = m_amp_db_audio - inv_sca_X_pred[:,:,:,0]
    #Reconstruct audio from denoised spectrogram and phase
    messageQueue.put("Reconstructing audio...")
    print(X_denoise.shape)
    print(m_pha_audio.shape)
    print(frame_length)
    print(hop_length_fft)
    audio_denoise_recons = matrix_spectrogram_to_numpy_audio(X_denoise, m_pha_audio, frame_length, hop_length_fft)
    messageQueue.put("Denoised audio shape: "+str(audio_denoise_recons.shape))
    #Number of frames
    ...
    messageQueue.put("Writing audio to output file...")
    print(dir_save_prediction)
    print(audio_output_prediction)
    sf.write(os.path.join(dir_save_prediction,audio_output_prediction),denoise_long[0, :], sample_rate,'PCM_24')
